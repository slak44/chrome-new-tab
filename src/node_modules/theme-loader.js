...
/*
  We absolutely need to force the loading of the theme before anything else, so on the first load we stick a precompiled
  version in localStorage (which blocks), so we can just load it immediately when available.
*/
const compiledThemeKey = 'localStorageCompiledTheme';

// returns true if they were loaded, false otherwise
function tryLoadingPrecompiledStyles() {
  const styleText = localStorage.getItem(compiledThemeKey);
  if (styleText !== null) {
...
function switchTheme(theme, cacheStyles) {
  ...
  if (cacheStyles) localStorage.setItem(compiledThemeKey, compiledText);
}

function clearCompiledCache() {
  localStorage.clear(compiledThemeKey);
}

export {
  tryLoadingPrecompiledStyles,
  switchTheme,
  clearCompiledCache
};
