...
/**
  ...
  Each key is a property name on the global object.
  Each stored item is represented by an object that has an `init` property, and a `default` property.
...
storage.loadAll = () => new Promise((resolve, reject) => {
  ...
  Object.keys(storage.elements).forEach(key => {
    window[key] = storage.elements[key].init;
    defaults[key] = storage.elements[key].default;
  });
  chrome.storage.local.get(defaults, items => {
    Object.keys(storage.elements).forEach(item => window[item] = items[item]);
    resolve(items);
  });
});
/**
...
*/
storage.storeAll = () => new Promise((resolve, reject) => {
  const toStore = {};
  Object.keys(storage.elements).forEach(e => toStore[e] = window[e]);
...
storage.clearAll = () => {
  Object.keys(storage.elements).forEach(element => delete window[element]);
