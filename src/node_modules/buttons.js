...
function newSettingCard(kind) {
  ...
  switch (kind) {
    case 'divider': stored.buttons.push({kind, position: ''}); break;
    case 'subheader': stored.buttons.push({kind, text: '', position: ''}); break;
    default: stored.buttons.push({
      text: '',
      href: '',
      pictureType: 'image',
      imagePath: '',
  ...
  addSettingCard(stored.buttons[stored.buttons.length - 1], stored.buttons.length - 1);
  $(`.card[data-button-idx="${stored.buttons.length - 1}"] input.focus-location`).focus();
  $(`.card[data-button-idx="${stored.buttons.length - 1}"] i.warning`).removeClass('hidden');
...
function removeWithUndoListener(card, idx) {
  card.find('.remove-action').click(event => {
    stored.buttons[idx].deleted = true;
    card.addClass('hidden');
    window.changesMade = true;
    undoToast('Deleted button', `button-${idx}`, () => {
      stored.buttons[idx].deleted = false;
...
function createBinderOnBlur(card, idx, propName, propKind = 'text') {
  card.find(`input[name="${propName}"]`).on('keyup change paste', event => {
    ...
    if (propKind === 'number') stored.buttons[idx][propName] = parseFloat(event.target.value);
    else stored.buttons[idx][propName] = event.target.value;
    window.changesMade = true;
    card.find('i.warning').removeClass('hidden');
  });
}

function createWarningTooltip(card) {
  card.find('i.warning').tooltip({
    delay: 50,
    position: 'bottom',
...
function addDividerCard(config, idx) {
  ...
  <div class="card" data-button-idx="${idx}">
    <div class="card-content">
      <span class="card-title">Divider <i class="material-icons warning hidden right">warning</i></span>
...
function addSubheaderCard(config, idx) {
  ...
  <div class="card" data-button-idx="${idx}">
    <div class="card-content">
      <span class="card-title">Subheader <i class="material-icons warning hidden right">warning</i></span>
      <div class="input-field">
...
function addButtonCard(config, idx) {
  ...
  <div class="card" data-button-idx="${idx}">
    <div class="card-content">
    <span class="card-title">Button <i class="material-icons warning hidden right">warning</i></span>
      <div class="input-field">
        <input class="focus-location" name="text" type="text" value="${config.text}">
        <label for="text" class="active">Text</label>
      </div>
      ...
      <div class="input-field ${config.pictureType === 'icon' ? '' : 'hidden'}">
        ...
        <div class="card-image">
          ...
          <i class="preview material-icons ${hasIcon ? '' : 'hidden'}">${config.ligatureName}</i>
        </div>
      </div>
      <div class="input-field ${config.pictureType === 'image' ? '' : 'hidden'}">
        ...
        <div class="card-image">
          ...
          <img src="${config.imagePath}" class="${hasImg ? '' : 'hidden'}">
        </div>
      </div>
      <div class="row">
  ...
  selectList.children().click(function () {
    ...
    const input = card.find('[name="href"]');
    input.val(actionUrl);
    input.focus();
    stored.buttons[idx].href = actionUrl;
    window.changesMade = true;
    card.find('i.warning').removeClass('hidden');
  ...
  card.find('input[name="use-icons"]').change(event => {
    ...
    stored.buttons[idx].pictureType = useIcons ? 'icon' : 'image';
    window.changesMade = true;
    card.find('i.warning').removeClass('hidden');
  });
  iconInput.on('keyup paste change', event => {
    stored.buttons[idx].ligatureName = event.target.value;
    window.changesMade = true;
    card.find('i.warning').removeClass('hidden');
  ...
  imageInput.on('keyup paste change', event => {
    stored.buttons[idx].imagePath = event.target.value;
    window.changesMade = true;
    card.find('i.warning').removeClass('hidden');
...
function sorted() {
  return [...stored.buttons].map((button, idx) => {
