...
pluginApi.getSetting = (plugin, setting) => plugin.settings.filter(settingObj => settingObj.name === setting)[0].value;
...
window.bindApi = pluginName => {
  const plugin = plugins.filter(plugin => pluginName === plugins.name);
  const api = Object.create(pluginApi);
  Object.keys(api).filter(key => api[key] instanceof Function).forEach(key => api[key] = api[key].bind(null, plugin));
  return api;
};
