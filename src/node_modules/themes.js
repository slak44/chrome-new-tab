...
function activateTheme(theme) {
  ...
  $('#dynamic-colors').text(`
    ...
    .btn, .btn-large {color: ${primaryTextColor} !important;}
    .btn, .btn-large {background-color: ${theme.accent} !important;}
    .btn:hover, .btn-large:hover {background-color: ${theme.selection} !important;}
    ...
    .switch label input[type=checkbox]:checked + .lever {background-color: ${theme.selection} !important;}
    .switch label input[type=checkbox]:checked + .lever:after {background-color: ${theme.accent} !important;}
    /* FIXME parse colors and add alpha correctly */
    input[type=checkbox]:not(:disabled) ~ .lever:active:before {background-color: ${theme.accent}26 !important;}
  `);
}
...
function addThemeSettingsUI(theme, idx) {
  ...
  <div class="theme-data hidden" data-theme-idx="${idx}">
    ...
    <div class="switch">
      <span class="primary-text">Light Theme</span>
      <label>
        <input name="theme-is-dark" type="checkbox" ${theme.isDark ? 'checked' : ''}>
        <span class="lever"></span>
      </label>
      <span class="primary-text">Dark Theme</span>
    </div>
    ...
    <div class="input-field">
      <input name="theme-selection" type="color" value="${theme.selection}">
      <label for="theme-selection" class="active">Selection Color</label>
    </div>
    <div class="input-field">
      <input name="theme-lighten" type="color" value="${theme.lighten}">
      <label for="theme-lighten" class="active">Lighter Main Color</label>
    </div>
    <div class="input-field">
      <input name="theme-darken" type="color" value="${theme.darken}">
      <label for="theme-darken" class="active">Darker Main Color</label>
    </div>
  </div>
  `);
  const div = $(`#themes-container > [data-theme-idx="${idx}"]`);
  ['name', 'main', 'background', 'accent', 'selection', 'lighten', 'darken'].forEach(propName => bindOnChange(propName, idx));
